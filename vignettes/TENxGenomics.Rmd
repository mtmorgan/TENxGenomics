...
Simple or rich input is useful when wishing to work with only a small
portion of the data, especially during exploratory phases of
...
library(BiocFileCache)
bfc <- BiocFileCache()
path <- bfcrpath(bfc, "1M_neurons_neurons20k")
if (!file.exists(path)) {
    url <- paste0(
        "https://s3-us-west-2.amazonaws.com/10x.files",
        "/samples/cell/1M_neurons",
...
For this data set, a faster solution is to simply read the entire data
into memory
